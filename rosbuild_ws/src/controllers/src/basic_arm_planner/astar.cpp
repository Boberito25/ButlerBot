...
std::vector<PState*> Astar::run(int* start, double* target){
  ...
  compute_ik(target[0],target[1],target[2],target1,target2);

    // std::cout << "Initialize Start\n";
  compute_fk(start[0], start[1], start[2]);
  State* starts = &space[start[0]][start[1]][start[2]];
  starts->heuristic = heuristic(&space[start[0]][start[1]][start[2]]);
  starts->prev[0] = -1;
  starts->prev[1] = -1;
  starts->prev[2] = -1;
  ...
  while(!frontier.empty() && will_continue(current)){
    ...
    current = &space[pcur->state[0]][pcur->state[1]][pcur->state[2]];
...
bool Astar::will_continue(State* current){
  bool hitT1 = current->id[0] == target1[0] &&
               current->id[1] == target1[1] &&
               current->id[2] == target1[2];
