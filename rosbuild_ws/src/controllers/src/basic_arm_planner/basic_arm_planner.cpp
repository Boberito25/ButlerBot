...
bool Basic_Arm_Planner::move(
	...
	Astar planner;
	int[3] start = {req.theta1, req.theta2, req.theta3};
	double[3] target = {req.x, req.z, req.alpha};
	...
	for(int i = 0; i < len; i++){
		res.theta1s.push_back(ticks_to_radians(path.at(i).state[0], planner.numticks));
		res.theta2s.push_back(ticks_to_radians(path.at(i).state[1], planner.numticks));
		res.theta3s.push_back(ticks_to_radians(path.at(i).state[2], planner.numticks));
	}
...
int main(int argc, char **argv){
	...
	ros::ServiceServer planner = c.n.advertise("Basic_Arm_Mover", &Basic_Arm_Planner::move, &c);
	ros::spin();
}